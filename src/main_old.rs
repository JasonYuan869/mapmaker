...
use num::Integer;
use image_processor::colors::{MINECRAFT_COLOR_TREE};
...
fn make_nbt(
    ...
    let mut source = match image::open(source_path) {
        Ok(image) => image,
        Err(e) => {
    ...
    let pixel_array = convert_colors(&mut resized, &MINECRAFT_COLOR_TREE, map_width as usize);
...
fn convert_colors(image: &mut RgbImage, map: &RgbColorMap, map_width: usize) -> Vec<i8> {
    ...
    // Each subsequence of 16384 entries represent one single map
